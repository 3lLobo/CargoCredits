...
export async function transferCargoCredit(addressFrom: string, addressTo: string, amount: string): Promise<void> {
  const provider = new ethers.providers.JsonRpcProvider("https://api.hyperspace.node.glif.io/rpc/v1");
  const wallet = new ethers.Wallet(process.env.PRIVATE_HYPERSPACE_KEY, provider);
