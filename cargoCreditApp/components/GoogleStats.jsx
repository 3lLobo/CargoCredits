    ...
    <div className="w-full h-full grid grid-flow-row sm:grid-flow-col gird-cols-3">
      <div className="col-span-2">
        <div className="flex flex-col justify-evenly w-full py-1 px-4 sm:px-6 lg:px-6 lg:py-11">
          <div className="flex ml-[15vw] w-full items-center">
          ...
          >
            <div className="lg:col-span-1">
              <ul
                distance="list"
                className="space-y-12 sm:grid sm:grid-cols-1 sm:gap-6 sm:space-y-0 lg:gap-x-8"
              >
                {travelStats
                  ...
                  .map((ele) => (
                    <li
                      ...
                      className={clsx([
                        ele.distance
                          .filter((el) => el.month === month.slice(0, 3))
                          .map((el) => el.distance)[0] == 0
                          ? "opacity-0"
                          : "opacity-100",
                      ])}
                      ...
                      <div className="flex flex-row items-center space-x-4 lg:space-x-6">
                        <div
                          className={clsx([
                            ...
                            ele.green
                              ? " ring-emerald-300 bg-emerald-100"
                        ...
                        <div className="space-y-1 text-md font-bold leading-6 ">
                          ...
                          <p className="text-ccDarkText">
                            {
                              ele.distance
                                .filter((el) => el.month === month.slice(0, 3))
                                .map((el) => el.distance)[0]
                            }{" "}
                            km
                          </p>
                        </div>
      ...
      {month !== "Choose a Month!" && (
        <div className="flex mx-auto col-span-1 w-full">
          <ConvertCredits
            distance={travelStats
              .filter((el) => {
                if (el.green) {
                  return el;
                }
              ...
              .map(
                (el) =>
                  el.distance
                    .filter((ele) => ele.month === month.slice(0, 3))
                    .map((ele) => ele.distance)[0]
              )
              .reduce((a, b) => a + b, 0)}
            month={month}
          />
